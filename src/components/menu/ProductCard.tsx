...
export function ProductCard({ item, width, onAdd }: Props) {
  return (
    <Pressable onPress={() => onAdd(item)} style={({ pressed }) => [styles.card, { width }, pressed && styles.pressed]}>
      <View style={[styles.art, { backgroundColor: item.color }]}><AppIcon name={item.icon} color="rgba(23,32,27,0.58)" size={39} /></View>
      <View style={styles.info}>
        <Text numberOfLines={1} style={styles.name}>{item.name}</Text>
        <View style={styles.bottomRow}><Text style={styles.price}>${item.price.toFixed(2)}</Text><View style={styles.add}><AppIcon name={{ ios: "plus", android: "add", web: "add" }} color={colors.primary} size={18} /></View></View>
      </View>
    </Pressable>
  );
}
